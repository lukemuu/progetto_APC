...
class HackerDashboard(ctk.CTk):
    ...
    def __init__(self):
        ...
        self._packets: list[dict] = []          # {id, raw, hex, time, replayed, replay_count}
    ...
    def _section(self, parent, text, row):
        f = ctk.CTkFrame(parent, fg_color=C["border"], height=1)
        f.grid(row=row, column=0, sticky="ew")
        lbl = ctk.CTkLabel(parent, text=text, font=FONT_TITLE,
                           text_color=C["text_dim"])
        lbl.grid(row=row, column=0, padx=12, pady=(10,2), sticky="w")
    ...
    def _build_packet_list(self):
        ...
        self._pkt_frame = ctk.CTkScrollableFrame(
            container, fg_color=C["bg"], scrollbar_button_color=C["border"],
    ...
    def _add_packet_card(self, packet: dict):
        ...
        strip.grid(row=0, column=0, rowspan=3, sticky="ns", padx=(0,8))
        packet["strip"] = strip
        ...
        id_lbl = ctk.CTkLabel(hdr, text=f"PKT  ", font=FONT_UI_SM, text_color=C["text_dim"])
        ...
        hex_lbl = ctk.CTkLabel(card, text=packet["hex"], font=FONT_MONO_SM,
                                ...
                                wraplength=580)
        hex_lbl.grid(row=1, column=1, sticky="ew", pady=(0,6))

        # click → seleziona
        for w in [card, strip, hex_lbl, hdr, id_lbl, id_num]:
    ...
    def _refresh_card(self, packet: dict):
        ...
        if "card" in packet and packet["id"] != self._selected_idx:
            packet["card"].configure(border_color="rgba(245,158,11,0.3)" if packet["replayed"] else C["border"])

    # ── PANNELLO DESTRO ───────────────────────────────────────────────────────
    def _build_right_panel(self):
        rp = ctk.CTkFrame(self, fg_color=C["panel"], corner_radius=0, width=300)
        ...
        self._detail_frame = ctk.CTkScrollableFrame(
            rp, fg_color="transparent", height=260,
    ...
    def _render_detail(self, p: dict):
        ...
        def field(label, value, color=C["text"], row=0):
            ctk.CTkLabel(df, text=label, font=FONT_TITLE,
                         text_color=C["text_dim"]).grid(row=row*2, column=0, sticky="w", pady=(8,2))
            ctk.CTkLabel(df, text=value, font=FONT_MONO_SM, text_color=color,
                         anchor="w", justify="left", wraplength=250).grid(
        ...
        field("HEX DUMP",      "",                                     C["text"],   2)

        # hex grid
        grid_f = ctk.CTkFrame(df, fg_color="transparent")
        grid_f.grid(row=5, column=0, sticky="ew", pady=(0,4))
        for i, b in enumerate(p["raw"]):
            hex_s = f"{b:02X}"
            if b == 0:
                color = C["text_dim"]
            elif 32 <= b < 127:
                ...
                color = C["text_mid"]
            lbl = ctk.CTkLabel(grid_f, text=hex_s, font=("Courier New", 10),
                               text_color=color, width=28, height=22,
                               fg_color=C["bg"], corner_radius=2)
            lbl.grid(row=i//8, column=i%8, padx=1, pady=1)

        if p["replayed"]:
            field("RITRASMISSIONI", f"{p['replay_count']}×", C["warn"], row=10)
    ...
    def _rx_loop(self, psize: int):
        ...
        while not self._stop_rx.is_set():
            ...
            self._stats["rx"] += 1
            log.debug(f"RX #{packet['id']+1:04d} | {packet['hex']}")
    ...
    def _on_packet_received(self, packet: dict):
        ...
        self._update_stats()
        self._log(f"RX #{packet['id']+1:04d} — {len(packet['raw'])} byte ricevuti")
    ...
    def _execute_replay(self, packet: dict, n: int):
        ...
        for i in range(n):
            try:
                ...
                self._ser.flush()
                self._stats["tx"] += 1
                log.debug(f"TX #{self._stats['tx']:04d} | {scritti} byte | replay #{packet['id']+1:04d}")
                self._log(
    ...
    def on_close(self):
        ...
        if self._ser and self._ser.is_open:
            self._ser.close()
        log.info(f"Chiusura — RX:{self._stats['rx']} TX:{self._stats['tx']} ERR:{self._stats['err']}")
        self.destroy()
